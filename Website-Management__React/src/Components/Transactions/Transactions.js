...
function Transactions(props) {
    ...
    async function returned(index){
        let temp_object = {}
        await firebase.firestore().collection('cards').doc(arrayOfObjects[index].cardId).get().then( (snapshot) => { // eslint-disable-line no-loop-func
        ...
        //
        const cardRef = firebase.firestore().collection('cards').doc(arrayOfObjects[index].cardId)   
